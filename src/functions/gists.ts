...
export async function createGist(options: CreateGist): Promise<CreateGistResponse> {
  const res = await (
    await get(`gists`, {
      method: "POST",
 ...
 * Update a gist by id
 */
export async function updateGist(id: string, options: CreateGist): Promise<CreateGistResponse> {
  const res = await (
    await get(`gists/${id}`, {
      method: "PATCH",
      body: JSON.stringify(options),
 ...
 */
export async function viewGists(options?: ViewGists): Promise<ViewGistsResponse[]> {
  const res = await (
    await get(`gists`, {
      method: "GET",
      needsToken: true,
